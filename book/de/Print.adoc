...
Das Diagramm aus dem Tutorial 1 (<<sec:tutorial1>>) sieht exportiert als PNG-Bild so aus:
...
[TIP]
====
Die Funktion `QWidget::grab()` macht bei jedem beliebigen Qt Widget einen Screenshot oder bei Verwendung von `QWidget::grab(QRect(...))` einen Teil-Screenshot des Widgets und aller seiner Kind-Widgets. Das resultierende QPixmap kann dann beliebig weiterverarbeitet werden.
====
...
Statt das Pixmap in eine Datei zu speichern, kann man das auch einfach in die Zwischenablage kopieren. Dazu `QClipboard` und `QApplication` einbinden und:
...
Beim Drucken wird das Bild einfach auf eine Druckerzeichenfläche gerendert. Für die Druckunterstützung muss im Qt-Programm zunächst die pro-Datei erweitert werden:
...
Ein generiertes SVG-Dokument kann man auch in die Zwischenablage kopieren. Dazu legt man im `QSvgGenerator` ein alternatives Ausgabegerät an (mit `QSvgGenerator::setOutputDevice()`) und
...
Die Skalierung von Schriftgrößen und Linienbreiten wird über die Auflösung (angegeben in DPI) des gewählten Zeichengeräts festgelegt. Zusammen mit der Pixel-Größe des Plots ergibt sich dann das finale Erscheinungsbild des Diagramms, welches durchaus von dem Bild auf dem Bildschirm abweichen kann.
