...
Beim PDF-Export des Diagramms mit der Methode oben, stellt man bei der Anzeige fest, dass die Skalenlinien nur kosmetische Linien mit einem Pixel Breite sind (d.h. beim Hineinzoomen bleiben diese Linien immer exakt ein Pixel breit). Alle anderen Diagrammelemente werden entsprechend skaliert.
...
Auch hier darf man nicht vergessen, den Zeichenstift (`QPen`)  entsprechend zu setzen.
