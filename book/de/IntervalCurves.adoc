...
[TIP]
====
Gestapelte Kurven kann man klassisch auch mit gefüllten Linienkurven machen (siehe <<sec:filledCurves>>), welche übereinander gezeichnet werden. Dazu müssen die Linen einen Brush erhalten und von hinten nach vorne gezeichnet werden. Mittels `QwtPlotItem::setZ()` kann man den Tiefenwert setzen - je höher der Wert, umso weiter vorne ist das Zeichenelement.
====
...
Gestapelte Kurven entstehen ja durch as _Stapeln_ normaler Dateireihen. Diese müssen nicht zwingend die gleichen X-Werte pro Datenreihe verwenden und können positive und negative Zahlen haben. Um solche Daten korrekt zu Stapeln und Anzuzeigen ist etwas Vorbereitungsarbeit notwendig.
...
.Drei Datenreihen mit positive und negativen Zahlenwerten
image::IntervalCurves/StackedCurve_originalDataSeries.png[pdfwidth=10cm]

### Nulldurchgänge bestimmen

Als erstes muss man die Nulldurchgänge der Kurven bestimmen und an diesen Stellen neue Stützstellen mit y=0 einfügen. Das macht eine Funktion `addZeroCrossings()`:
...
Um die Datenwerte addieren zu können, müssen die Stützstellen übereinstimmen. Also wird ein einheitliches X-Werte-Raster bestimmt und die jeweiligen fehlenden Werte interpoliert.
