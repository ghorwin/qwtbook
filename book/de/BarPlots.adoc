...
### Balkendigramme mit Lücken zwischen den Balken oder unregelmäßigen Abständen
...
Wenn man in einem Diagram mehrere `QwtPlotBarChart` Objekte integriert, zeichnet das `QwtPlot` diese unabhängig voneinander. Damit kann man beispielsweise positive und negative Balken an den gleichen X-Koordinaten zeichnen, oder sich überlappende Balken.
...
Bei den gestapelten Kurven sorgt der Symbolrahmen `QwtColumnSymbol::Plain` zu doppelten Trennstrichen zwischen den Balken (siehe Bild oben).
