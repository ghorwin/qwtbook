...
.Balkendiagramm im Layoutmodus 'ScaleSampleToCanvas', mit 10% Zeichenflächenbreite als Balkenbreite
...
.Balkendiagramm mit angepasstem Stil für die Balken
...
.Balkendiagramm mit schwarz umrandeten, einfarbigen Balken
...
Möchte man statt der Zahlen am unteren Bildschirmrand Beschriftungen für die einzelnen Balken haben, muss man die Klasse `QwtScaleDraw` ableiten und dann die virtuelle Funktion `QwtScaleDraw::label()` überschreiben.
...
### Balkendiagramme mit Lücken zwischen den Balken oder unregelmäßigen Abständen

Die Position der Balken, d.h. die X-Koordinaten, werden ja automatisch in der Funktion `setSamples()` festgelegt (0,1,2,3, ...). Die Balken werden dann zentriert um diese Koordinate gezeichnet (siehe auch Beispiele ganz oben).

Möchte man die Balken in anderen Abständen oder mit Lücken zwischen den Balken zeichnen, so kann man die überladene Funktion `setSamples( QwtSeriesData< QPointF >* )` verwenden:
...
.Balkendiagramm mit unregelmäßigen x-Koordinaten, ungünstige Balkenbreite
image::BarPlots/BarPlot_1_IrregularXCoordsOverlapping.png[pdfwidth=6cm]

Die Breite der Balken wird standardmäßig mit `QwtPlotAbstractBarChart::AutoAdjustSamples` bestimmt, was zu den oben gezeigten überlappenden Balken führt. Besser ist es, den Berechnungsmodus auf 
...
.Balkendiagramm mit unregelmäßigen x-Koordinaten (diesmal mit vernünftigen Balkenbreiten)
image::BarPlots/BarPlot_1_IrregularXCoords.png[pdfwidth=6cm]


### Mehrere Balkendiagramme im gleichen Diagramm

Wenn man in einem Diagramm mehrere `QwtPlotBarChart` Objekte integriert, zeichnet das `QwtPlot` diese unabhängig voneinander. Damit kann man beispielsweise positive und negative Balken an den gleichen X-Koordinaten zeichnen, oder sich überlappende Balken.

Dazu erstellt und konfiguriert man einfache mehrere `QwtPlotBarChart` Objekte:
...
Die Plots werden standardmäßig in der Reihenfolge gezeichnet, wie sie dem Plot hinzugefügt wurden. Man kann das aber auch über das Setzen der z-Reihenfolge ändern (siehe auch <<sec:plotitems>>). 

.Balkendiagramm mit zwei BarChart PlotItems, wobei eines nur positive und das andere nur negative Balken zeichnet
...
Bisher gab es für jede X-Koordinate genau einen Balken, bzw. einen positiven und einen negativen Balken, falls man zwei `QwtPlotBarChart`-Objekte verwendet hat. Man kann allerdings auch Gruppen von Balken zeichnen, und damit 3D-Informationen abbilden.

Dafür verwendet man das Zeichenelement `QwtPlotMultiBarChart`. Dieses teilt sich eine Reihe von Eigenschaften mit dem vorab besprochenen `QwtPlotBarChart`. Statt aber zentrisch um eine X-Koordinate _einen Balken_ zu zeichnen, kann dieses Zeichenelement mehrere Balken zeichnen, und zwar:

- nebeneinander
- gestapelt übereinander (nur ausschließlich positive oder ausschließlich negative Werte)

Die Daten werden in einer 3D-Datenstruktur bereitgestellt, wie im folgenden Beispiel (Stadt, Jahr, Anzahl):

.Quelldaten für ein `QwtPlotMultiBarChart`-Diagramm
image::BarPlots/MultiBarPlot_SourceData.png[pdfwidth=6cm]

Das dazugehörige Diagramm wird wie folgt konfiguriert:
...
// QwtPlotMultiBarChart erstellen
QwtPlotMultiBarChart *barChart = new QwtPlotMultiBarChart;
// gruppierte Darstellung wählen
...
.`QwtPlotMultiBarChart`-Diagramm mit gruppierten Balken
image::BarPlots/MultiBarPlot_grouped.png[pdfwidth=6cm]


Alternativ kann man auch auf gestapelte Diagrammdaten umstellen:

```c++
barChart->setStyle(QwtPlotMultiBarChart::Stacked);
```

.`QwtPlotMultiBarChart`-Diagramm mit gestapelten Balken
image::BarPlots/MultiBarPlot_stacked.png[pdfwidth=6cm]


### Andere Rahmen der gestapelten Kurven

Bei den gestapelten Kurven sorgt der Symbolrahmen `QwtColumnSymbol::Plain` für doppelte Trennstriche zwischen den Balken (siehe Bild oben).
Man kann natürlich auch den Stil ändern:
