...
Man kann die Klasse `QwtWeedingCurveFitter` jedoch auch zu Datenreduktion _außerhalb_ der QwtPlotCurve verwenden, d.h. man reduziert die der Kurve übergebenen Daten vorab. Und hier liegt das wirkliche Performancesteigerungspotential.
...
Nun benötigt der Algorithmus einmalig ca. **800 ms**, aber das Zeichnen auf dem Bildschirm und Aktualisieren des Plots beim Plot-Größenänderungen dauert minimale **2 ms**, und _das_ ist eine wahrhaftige Performancesteigerung.
