...
In keiner Variante ist die Option _mit_  schneller. Außerdem fällt auf, dass unabhängig von der Option `FilterPoints` der CurveFitter immer alle 10 Mio Punkte übergeben bekommt, so wie auch der Polygon-Clipping-Algorithmus. 
...
Auch wenn der `QwtWeedingCurveFitter` letztlich das Polygon sehr stark verkleinert, ist der Mehraufwand in den Algorithmen durch den Wegfall des Punktefilterns enorm.
