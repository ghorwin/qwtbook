...
Man kann die Zeichengeschwindigkeit des `QwtPlot` erhöhen, wenn man bei sehr großen Reihendaten nur die Daten wirklich zeichnet, die sich auf das Diagramm auswirken. Wenn bspw. 100 Datenpunkte auf dem gleichen Pixel landen, braucht man die nicht alle zeichnen. Das Ausfilter nicht sichtbarer Punkte und Liniensegmente hängt natürlich von der Zoomstufe ab und ist daher auch als Kurvenfilter implementiert.

Die Basisklasse für solche Filter ist die Klasse `QwtWeedingCurveFitter`.
