...
Die Klasse `QwtPlotCurve` hat verschiedene Zeichenattribute, die verwendete Algorithmen näher konfigurieren. Hier ist zunächst eine kurze Liste, wobei die individuelle Attribute und deren Auswirkung später näher beschrieben werden:

- `ClipPolygons` - damit werden zu zeichnende Datenreihen ausgedünnt, sodass nur im aktuellen Zoomfenster sichtbare Kurventeile gezeichnet werden. Ist standardmäßig aktiv und kann in bestimmten Situationen die Renderperformance verbessern, bzw. ist z.B. für SVG-Export notwendig, führt aber in den meisten Standardanwendungsfällen zu einer minimalen Verlangsamung des Renders.
- `FilterPoints` - damit werden Punkte aus den Datenreihen herausgefiltert, die sowieso nicht gezeichnet werden würden.
- `FilterPointsAggressive` - Nur für Linienkurven, ersetzt viele kurze übereinanderliegende Polygonstücke durch einzelne Liniensegmente
- `MinimizeMemory` - Verhindert wenn eingeschaltet und _nur_ beim Linientyp `QwtPlotCurve::Dots`, dass bei der Transformation der Datenreihe von Plotkoordinaten zu Renderkoordinaten eine eventuell große Polygon-Kopie erstellt wird. Stattdessen werden alle Punktkoordinaten einzeln transformiert, geclippt und gezeichnet. Da hierbei für jeden Punkt ein gewisser Extraaufwand benötigt wird, ist dies in der Regel etwas langsamer.
- `ImageBuffer` - Eine sehr spezielle Optimierung _nur_ beim Linientyp `QwtPlotCurve::Dots`, wobei die Punkte (jeweils nur ein Pixel pro Punkt) in ein Bild gezeichnet werden. Dies erfolgt parallelisiert und kann daher sehr schnell sein, ist allerdings nur effektiv bei sehr großen Punktmengen.
...
[NOTE]
====
Neben dem Filtern überflüssiger Punkte durch Setzen des Zeichenattributs `FilterPoints` gibt es auch noch die Möglichkeit, den `QwtWeedingCurveFitter` (siehe <<sec:weedingCurveFitter>>) zu verwenden. Dieser ist aber eigentlich für die Datenreduktion beim Export gedacht und verlangsamt das Zeichnen auf dem Bildschirm in der Regel.
====
...
reduziert sich die Zeichenzeit um immerhin 25% auf ca. **730 ms**.

[TIP]
====
Man kann das standardmäßig gesetzte Zeichenattribut `QwtPlotCurve::ClipPolygons` beim Rendern auf dem Bildschirm abschalten, um so die Zeichengeschwindigkeit zu erhöhen. Da das Zeichnen sowieso auf die Canvas-Fläche begrenzt ist, braucht man das vorherige Ausdünnen des Polyogons nicht wirklich. Sinnvoll ist die `ClipPolygons`-Funktion unter Umständen bei der Verwendung von Kurvenfilter-Algorithmen, bzw. beim SVG-Export.
====
