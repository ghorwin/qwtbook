...
Eine grundlegende Eigenschaft der `QwtPlot`-Klasse ist die Besitzübernahme hinzugefügter Elemente. Dies gilt allgemein für alle Elemente des Plots (Linien, Marker, Legende, ...). D.h. nach Übertragung der Eigentümerschaft kümmert sich das `QwtPlot` um das Aufräumen des Speichers.
...
[WARNING]
====
Die API der Qt Designer Plugins hat sich in jüngeren Qt Creator-Versionen geändert, weswegen die Qwt Designer Plugins mit aktuellen Qt und Qt Creator Versionen nicht mehr funktionieren, selbst wenn man sie mit den passenden Bibliotheksversionen kompiliert.
====
